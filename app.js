...
while (again) {
    while (total <= 5) {
        do {
            ...
            if (userNum == secretNum || compNum == secretNum) {
                ...
                // console.log(winner)
                winner === "you"
                    ? chalkAnimation.neon(`${winner} win! secret number was ${secretNum}\n\n`, 2)
                    : chalkAnimation.glitch(`${winner} win! secret number was ${secretNum}\n\n`);
                await sleep();
                winner == "you" ? UserScore++ : compScore++;
                let userScoreString = chalk.bgGreen.white.bold(`Player score: ${UserScore}`);
                let compScoreString = chalk.bgBlue.whiteBright.bold(`computer score: ${compScore}`);
                total++;
                if (total <= 5) {
    ...
    if (UserScore > compScore) {
        chalkAnimation.neon(`Player WIN! ${UserScore}/5`);
    }
    else if (UserScore < compScore) {
        chalkAnimation.glitch(`Computer WIN! ${compScore}/5`);
    }
    await sleep();
...
function game() {
    ...
    console.log(`your Guessed = ${chalk.green.bold(userNum)} computer Guessed = ${chalk.blue.bold(compNum)}\n`);
}
